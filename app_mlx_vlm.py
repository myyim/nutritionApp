...
* For each image provided, identify all food items, estimate their quantity, and calculate a nutritional breakdown. 
...
def run_model(prompt,image=[],audio=[]):
    # Apply chat template
    ...
    # Generate output
    output = generate(model, processor, formatted_prompt, image, audio=audio, max_tokens=2000, temperature=0.2, verbose=False) # prompt_cache=prompt_cache
    return output.text
